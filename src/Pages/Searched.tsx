...
function Searched() {   
  const [searched, setSearched] = useState<SearchTypes[]>([])
   ...
   const {id} = useParams()  
   let url = `https://newsapi.org/v2/everything?q=${id}&sortBy=publishedAt&apiKey=b254967dd62e4b51b3d79d2c8f245083`
  ...
  setSearched(data.articles)
   ...
   if(searched.length === 0) {
    return (
      <div className="text-xl text-center my-7">zero results</div>
    )
   }
        ...
        {searched.map((item) => {
          return (
            <a key={item.id} className="border border-slate-200" href={item.url}> 
              ...
              <h1 className='my-2 font-light px-3'>{item.source.name}</h1>  
            </a>
          )
        })}
       </div>
